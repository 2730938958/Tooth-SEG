...
def predict_from_raw_data(list_of_lists_or_source_folder: Union[str, List[List[str]]],
    ...
    sess = rt.InferenceSession('nnunetv11-pth1.onnx')
                        ...
                        for params in parameters:
                            ...
                            if prediction is None:
                                prediction = predict_sliding_window_return_logits(
                                    network, sess, data, num_seg_heads,
...
def predict_entry_point():
    ...
    args = parser.parse_args()
    # args = parser.parse_args(args=['-i', '/home/HwHiAiUser/samples/notebooks/nnunet/data', '-o',
    #                                '/home/HwHiAiUser/samples/notebooks/nnunet/output', '-d', '1', '-c', '3d_lowres',
    #                                '-f', '0', '-device', 'cpu', '--save_probabilities'])
    # print(args)
    args.f = [i if i == 'all' else int(i) for i in args.f]
