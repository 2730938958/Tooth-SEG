...
def maybe_mirror_and_predict(network: nn.Module, sess, x: torch.Tensor, mirror_axes: Tuple[int, ...] = None) \
        -> torch.Tensor:
    # prediction = network(x)
    # x = x.half()
    input_name = sess.get_inputs()[0].name
    output_name = sess.get_outputs()[0].name
    # output = sess.run([output_name], {input_name: x.numpy()})
    pred = torch.squeeze(torch.tensor(np.array(sess.run([output_name], {input_name: x.numpy()}))), dim=0)
    # torch.save(x, "./x.pt")
    ...
    if mirror_axes is not None:
        ...
        if 0 in mirror_axes:
            ...
            pred += torch.flip(
                torch.squeeze(torch.tensor(np.array(sess.run([output_name], {input_name: torch.flip(x, (2,)).numpy()}))),
                              dim=0), (2,))

        if 1 in mirror_axes:
            # prediction += torch.flip(network(torch.flip(x, (3,))), (3,))
            pred += torch.flip(
                torch.squeeze(torch.tensor(np.array(sess.run([output_name], {input_name: torch.flip(x, (3,)).numpy()}))),
                              dim=0), (3,))

        if 2 in mirror_axes:
            # prediction += torch.flip(network(torch.flip(x, (4,))), (4,))
            pred += torch.flip(
                torch.squeeze(torch.tensor(np.array(sess.run([output_name], {input_name: torch.flip(x, (4,)).numpy()}))),
                              dim=0), (4,))

        if 0 in mirror_axes and 1 in mirror_axes:
            # prediction += torch.flip(network(torch.flip(x, (2, 3))), (2, 3))
            temp = torch.flip(x, (2, 3))
            pd = torch.tensor(np.array(sess.run([output_name], {input_name: temp.numpy()})))
            pred += torch.flip(
                torch.squeeze(pd,
                              dim=0), (2, 3))
    ...
    with torch.no_grad():
        ...
        with torch.autocast(device.type, enabled=True) if device.type == 'cuda' else dummy_context():
            ...
            for sl in tqdm(slicers):
                ...
                prediction = maybe_mirror_and_predict(network, sess, workon, mirror_axes)[0].to(results_device)
